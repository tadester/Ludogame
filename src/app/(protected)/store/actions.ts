...
export async function purchasePower(formData: FormData) {
  const itemId = String(formData.get("itemId") ?? "");
  if (!itemId) {
    storeRedirect("That power is no longer available.");
  }
  ...
  const { error } = await supabase.rpc("purchase_power", {
    p_item_id: itemId,
  });
  if (error) {
    if (error.code === "23514") storeRedirect("You don't have enough coins.");
    if (error.code === "23505") storeRedirect("You already own that power.");
    storeRedirect("Unable to complete the purchase. Try again.");
  }

  revalidatePath("/store");
  storeRedirect("Power unlocked! Equip it in your strategy book.");
}
